...
class AudioProcessor:
    ...
    def _generate_visemes_from_phonemes(self, audio_path: str) -> List[Dict[str, float]]:
        ...
        predicted_ids = torch.argmax(logits, dim=-1)[0].numpy().tolist()  # Convert to list to ensure iterability
...
class VisemeGenerator:
    ...
    def apply_viseme(self, neutral_landmarks: np.ndarray, viseme: str, intensity: float = 1.0) -> np.ndarray:
        ...
        print(f"Applying viseme {viseme} with config shape {viseme_config.shape} to landmarks shape {neutral_landmarks.shape}")

        if viseme_config.shape[0] != neutral_landmarks.shape[0]:
            print(f"Warning: Resizing viseme config for '{viseme}'")
            if viseme_config.shape[0] < neutral_landmarks.shape[0]:
                padding = np.zeros((neutral_landmarks.shape[0] - viseme_config.shape[0], 2))
                viseme_config = np.vstack([viseme_config, padding])
            else:
                viseme_config = viseme_config[:neutral_landmarks.shape[0], :]

        viseme_offset = viseme_config * intensity
        new_landmarks = neutral_landmarks + viseme_offset

        print(f"Landmark change for viseme {viseme}: min offset {np.min(viseme_offset)}, max offset {np.max(viseme_offset)}")
    ...
    def generate_default_viseme_configs(self):
        ...
        # Ensure all viseme configs have the correct shape
        self.viseme_configs = {
            "rest": np.zeros((num_landmarks, 2)),
            "open": np.array([[0, 0.02] if i % 2 == 0 else [0, -0.02] for i in range(num_landmarks)]),
            "wide_open": np.array([[0, 0.03] if i % 2 == 0 else [0, -0.03] for i in range(num_landmarks)]),
            "slight_open": np.array([[0, 0.01] if i % 2 == 0 else [0, -0.01] for i in range(num_landmarks)]),
            "round": np.array([[-0.01 if i < num_landmarks//2 else 0.01, 0.01] if i % 2 == 0 
                            else [0.01 if i < num_landmarks//2 else -0.01, -0.01] for i in range(num_landmarks)]),
            "bilabial": np.array([[0, 0.005] if i % 2 == 0 else [0, -0.005] for i in range(num_landmarks)]),
            "labiodental": np.array([[0, 0] if i < num_landmarks//2 else [0, 0.01] for i in range(num_landmarks)]),
            "dental": np.array([[0, 0] if i < num_landmarks//3 else [0, 0.01] for i in range(num_landmarks)]),
            "alveolar": np.array([[0, 0.01] if i % 3 == 0 else [0, 0] for i in range(num_landmarks)]),
            "palato-alveolar": np.array([[0, 0.015] if i % 4 == 0 else [0, 0] for i in range(num_landmarks)]),  # Added for completeness
            "velar": np.array([[0, 0.01] if i % 5 == 0 else [0, 0] for i in range(num_landmarks)]),  # Added for completeness
        }
            
        # Call this method in your __init__ function after defining mouth_indices
...
class FaceAnimator:
    ...
    def blend_visemes(self, viseme_sequence, frame_time, neutral_mouth_landmarks):
        print(f"Blending visemes at time {frame_time}, neutral landmarks shape: {neutral_mouth_landmarks.shape}")
        if not viseme_sequence or len(viseme_sequence) == 0:
            print("No visemes to blend, returning neutral")
        ...
        for v in viseme_sequence:
            ...
            if extended_start <= frame_time <= extended_end:
                weight = self._calculate_weight(frame_time, start, end, transition_time, v["intensity"])
                if weight > 0.05:
                    active_visemes.append({"viseme": v["viseme"], "weight": weight})
        ...
        for v in active_visemes:
            v_landmarks = self.viseme_generator.apply_viseme(neutral_mouth_landmarks, v["viseme"], 1.0)
            final_landmarks += v_landmarks * (v["weight"] / total_weight)

        print(f"Final blended landmarks min: {np.min(final_landmarks)}, max: {np.max(final_landmarks)}")
        return final_landmarks
    
    def _calculate_weight(self, frame_time, start, end, transition_time, intensity):
        if frame_time < start:
            return (frame_time - max(0, start - transition_time)) / transition_time * intensity
        elif frame_time > end:
            return (min(end + transition_time, end) - frame_time) / transition_time * intensity
        else:
            progress = (frame_time - start) / (end - start)
            if progress < 0.2:
                return progress / 0.2 * intensity
            elif progress > 0.8:
                return (1.0 - (progress - 0.8) / 0.2) * intensity
            else:
